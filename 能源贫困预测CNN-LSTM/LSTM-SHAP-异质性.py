# -*- coding: utf-8 -*-
"""
PyTorch版：按东/中/西部地区分别训练 LSTM 并绘制 SHAP 蜂群图 + 叠加条形图（风格与你给的一致）
- 数据列：AREA(省份简称如北京)、YEAR、EPI_entropy + 11个外生变量（中文列名）
- 特征显示：用简称（GDP, HC, SSI, PD, UR, EI, ECS, PGPC, FCR, LFEEP, WRPC）
- 输出：三张PNG：SHAP_East.png / SHAP_Central.png / SHAP_West.png（可改路径）
"""
...
# 0) 配置：文件路径 + 列名
# ======================
DATA_PATH = r"EPI_entropy_output.xlsx"  # <<< 你的文件
AREA_COL = "AREA"
YEAR_COL = "YEAR"
...
# ======================
# 2) 东/中/西部省份列表（不含西藏和港澳台）
# ======================
REGIONS = {
    "East": ["北京", "天津", "河北", "辽宁", "上海", "江苏", "浙江", "福建", "山东", "广东", "广西", "海南"],
    "Central": ["山西", "内蒙古", "吉林", "黑龙江", "安徽", "江西", "河南", "湖北", "湖南"],
    "West": ["四川", "贵州", "云南", "陕西", "甘肃", "宁夏", "青海", "新疆", "重庆"],
}
...
def read_main_sheet(path: str) -> pd.DataFrame:
    """优先读取 EPI_score sheet；否则读取第一个sheet，避免误读Weights/Standardized等。"""
...
def split_panel_train_test(df_in: pd.DataFrame, lookback: int, test_ratio: float):
    """按省份内部时间顺序构造序列样本，并按每省末尾test_ratio切测试集（不打乱）。"""
    Xtr, ytr, Xte, yte = [], [], [], []
    ...
    if len(Xtr) == 0 or len(Xte) == 0:
        raise ValueError("该分组下可用序列样本为空（可能某些省年份不足或数据缺失）。")
...
def compute_shap(model, X_train_s, X_test_s, lookback, n_feat):
    """计算SHAP值：优先Deep/Gradient，失败回退Kernel。"""
    ...
    # 汇总到特征维：对时间维取平均（与你之前一致）
    shap_feat = shap_values.mean(axis=1)            # (n_samples, n_feat)
    X_te_np = X_te.detach().cpu().numpy().mean(axis=1)  # (n_samples, n_feat)
    X_te_df = pd.DataFrame(X_te_np, columns=FEATURE_SYM)
...
# ======================
# 4) 主流程：分东/中/西部循环训练 + SHAP + 保存
# ======================
# 读取主数据sheet（避免读到Weights/Standardized）
...
data_all = df[need_cols].copy()

# 清洗：转数值，去掉异常行
data_all[YEAR_COL] = pd.to_numeric(data_all[YEAR_COL], errors="coerce")
data_all[TARGET_COL] = pd.to_numeric(data_all[TARGET_COL], errors="coerce")
for c in FEATURE_CN:
    data_all[c] = pd.to_numeric(data_all[c], errors="coerce")

data_all = data_all.dropna(subset=[YEAR_COL, TARGET_COL])
data_all[FEATURE_CN] = data_all[FEATURE_CN].fillna(data_all[FEATURE_CN].mean())

# 对每个区域分别建模+解释
for region_name, prov_list in REGIONS.items():
    print("\n" + "="*80)
    print(f"Region: {region_name} | Provinces: {prov_list}")

    # 过滤该区域数据
    data_r = data_all[data_all[AREA_COL].isin(prov_list)].copy()
    if data_r.empty:
        print(f"[Skip] {region_name} 无数据（请检查AREA是否为省份简称）。")
        continue

    # 构造训练/测试序列（按省内时间顺序）
    X_train, X_test, y_train, y_test = split_panel_train_test(data_r, LOOKBACK, TEST_RATIO)
    ...
    # 保存图像
    out_png = f"SHAP_LSTM_{region_name}.png"
    title = f"SHAP Summary ({region_name} Region)"
